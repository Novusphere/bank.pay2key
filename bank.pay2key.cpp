...
[[eosio::action]] void pay2key::transfer(
    uint64_t chain_id,
    ...
    check(quantity.amount + fee.amount >= quantity.amount, "invalid quantity or fee"); //overflow
    check(memo.size() <= 150, "memo has more than 150 bytes");
    ...
    uint8_t rawtx[255];
    memcpy(&rawtx[offset], (uint8_t *)&chain_id, 8);
    offset += 8;
    memcpy(&rawtx[offset], from.data.data(), 33);
    offset += 33;
    memcpy(&rawtx[offset], to.data.data(), 33);
    offset += 33;
    memcpy(&rawtx[offset], (uint8_t *)&quantity.amount, 8);
    offset += 8;
    memcpy(&rawtx[offset], (uint8_t *)&fee.amount, 8);
    offset += 8;
    memcpy(&rawtx[offset], (uint8_t *)&nonce, 8);
    offset += 8;
    memcpy(&rawtx[offset], memo.c_str(), memo.size());
    offset += memo.size();
